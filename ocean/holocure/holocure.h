...
void c_render(HoloCure* env) {
    ...
    Player* p = &env->player;
    DrawCircleV((Vector2){p->x * sx, p->y * sy}, 10, (Color){80, 200, 255, 255});
    if (env->attack.active)
        DrawCircleLinesV((Vector2){env->attack.x * sx, env->attack.y * sy},
                         WEAPON_RANGE * sx, (Color){255, 220, 120, 200});
    for (int i = 0; i < MAX_ENEMIES; i++) {
        Enemy* e = &env->enemies[i];
        if (!e->active) continue;
        DrawCircleV((Vector2){e->x * sx, e->y * sy}, 6, (Color){255, 80, 80, 255});
    }
    DrawText(TextFormat("HP %.0f  Kills %d  Step %d", p->hp, env->total_kills, env->step_count),
             10, 10, 20, RAYWHITE);
    EndDrawing();
}
